...
export const Dashboard : React.FC = () => {
    ...
    return (
        <Grid item container direction="column" spacing={5} className={classes.container}>
            <Grid item container wrap="nowrap" justify="space-between" alignItems="center">
                <Grid item>
                    <Typography variant="h5">My Orders</Typography>
                </Grid>
                <Grid item>
                    <Button variant="contained" color="primary" size="large" onClick={() => history.push('/new-order/1')}>New Order</Button>
                </Grid>
            </Grid>
            <Grid item>
                <Paper>
                    <Table stickyHeader>
                        ...
                        <TableBody>
                            {orders.length > 0 && orders?.map((entry, index) => (
                                <TableRow onClick={() => onClick(entry.id)} hover className={classes.pointer} key={`liensearch${index}`}>
                                    <TableCell>{entry.folio || '--'}</TableCell>
                                    <TableCell>{entry.address.address1 || '--'}</TableCell>
